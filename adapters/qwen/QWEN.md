# Global QWEN.md — Engineering Baseline

Project-local files **always override this baseline**:
`QWEN.md` · `AGENTS.md` · `README*` · `docs/` · `code-conventions.md` · `.editorconfig`
...
6. For large projects, keep the root context file lean. Move detailed domain knowledge into separate files and reference them from the root file.

---

## Coding Rules (always active)

The rule files below are imported into this context file and apply to every
session. Skills under `~/.qwen/skills/` are recommendations the model may load
on demand; these rules are binding.
